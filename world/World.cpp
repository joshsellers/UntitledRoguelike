...
sf::Image World::generateChunkTerrain(Chunk& chunk) {
    return _terrainGenerator.generateChunkTerrain(chunk, _seed);
}
...
void World::setRealm(REALM realm) {
    _terrainGenerator.setRealm(realm);
    _chunks.clear();
    _loadingChunks.clear();
    _scatterBuffer.clear();
    for (int i = 0; i < _chunkBuffer.size(); i++) {
        _chunkBuffer.pop();
    }

}

REALM World::getRealm() const {
    return _terrainGenerator.getRealm();
}
