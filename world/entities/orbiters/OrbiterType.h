...
public:
    ...
    OrbiterType(const unsigned int id, const std::string name, const sf::IntRect textureRect, const float orbitSpeed, const float orbitRadius, 
        ...
        const std::function<void(Orbiter*)> attack = [](Orbiter* orbiterInstance) {}, const bool onlyHitPlayer = false, const bool addVelocityToProjectiles = false);
    ...
    const bool addVelocityToProjectiles() const;
...
private:
    ...
    const bool _addVelocityToProjectiles = false;
};
