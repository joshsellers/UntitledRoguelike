...
BigSnowMan::BigSnowMan(sf::Vector2f pos) : Entity(BIG_SNOWMAN, pos, 2.f, 2 * TILE_SIZE, 3 * TILE_SIZE, false) {
    setMaxHitPoints(105);
    if (HARD_MODE_ENABLED) {
        //setMaxHitPoints(getMaxHitPoints() + ((float)getMaxHitPoints() * 1.15f));
    }
...
void BigSnowMan::update() {
    ...
    if (currentTimeMillis() - _lastFireTime >= fireRate) {
        fireTargetedProjectile(playerPos, ProjectileDataManager::getData("_PROJECTILE_LARGE_SNOWBALL"), "NONE", true);
        _lastFireTime = currentTimeMillis();
    }
}

void BigSnowMan::draw(sf::RenderTexture& surface) {
    ...
    if (isSwimming()) {
        ...
        _wavesSprite.setPosition(sf::Vector2f(getPosition().x - (float)TILE_SIZE / 2.f, getPosition().y + ((float)TILE_SIZE * 3) - 6));
    ...
    _sprite.setTextureRect(sf::IntRect(
        448 + xOffset, 368, TILE_SIZE * 2, isSwimming() ? (float)TILE_SIZE * 3.f / 2.f : TILE_SIZE * 3
    ));

    surface.draw(_sprite);
}

void BigSnowMan::loadSprite(std::shared_ptr<sf::Texture> spriteSheet) {
    _sprite.setTexture(*spriteSheet);
    _sprite.setTextureRect(sf::IntRect(448, 368, TILE_SIZE * 2, TILE_SIZE * 3));
    _sprite.setPosition(getPosition());
