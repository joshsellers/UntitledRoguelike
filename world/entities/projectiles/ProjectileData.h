...
struct ProjectileData {
    ProjectileData(const unsigned int itemId, const float baseVelocity, const sf::IntRect hitBox, const bool rotateSprite, const bool onlyHitEnemies = false,
        ...
        const bool allowDamageToProps = false) :
        ...
        useDamageMultiplier(useDamageMultiplier), explosionBehavior(explosionBehavior), allowPlayerProjectileEffects(allowPlayerProjectileEffects), allowDamageToProps(allowDamageToProps)
    {
    ...
    ProjectileData()
        ...
        explosionBehavior(EXPLOSION_BEHAVIOR::NOT_EXPLOSIVE), allowPlayerProjectileEffects(false), allowDamageToProps(false)
    { }
